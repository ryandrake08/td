...
{
  NSMutableString *s = [NSMutableString stringWithFormat:@"<%@:%p keyPath:%@", NSStringFromClass([self class]), (void*)self, _keyPath];
  if (0 != _options) {
  ...
  if (NULL != _block) {
    [s appendFormat:@" block:%p", (void*)_block];
  }
  [s appendString:@">"];
  return s;
}
...
- (NSString *)debugDescription
{
  NSMutableString *s = [NSMutableString stringWithFormat:@"<%@:%p", NSStringFromClass([self class]), (void*)self];
...
{
  NSMutableString *s = [NSMutableString stringWithFormat:@"<%@:%p", NSStringFromClass([self class]), (void*)self];
  [s appendFormat:@" observer:<%@:%p>", NSStringFromClass([_observer class]), (void*)_observer];
...
{
  NSAssert(0 != keyPath.length && NULL != block, @"missing required parameters observe:%@ keyPath:%@ block:%p", object, keyPath, (void*)block);
...
{
  NSAssert(0 != keyPaths.count && NULL != block, @"missing required parameters observe:%@ keyPath:%@ block:%p", object, keyPaths, (void*)block);
