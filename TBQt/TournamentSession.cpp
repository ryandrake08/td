...
{
    this->send_command("check_authorized", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result["authorized"].toBool());
    });
}
...
{
    this->send_command("get_state", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result);
    });
...
{
    this->send_command("get_config", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result);
    });
...
{
    this->send_command("configure", config, [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result);
    });
...
{
    this->send_command("set_previous_level", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result["blind_level_changed"].toInt());
    });
}
...
{
    this->send_command("set_next_level", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result["blind_level_changed"].toInt());
    });
...
{
    this->send_command("gen_blind_levels", request, [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result["blind_levels"].toList());
    });
}
...
{
    this->send_command("quick_setup", QVariantMap(), [handler](const QVariantMap& result)
    {
        if(handler)
            handler(result["seated_players"].toList());
    });
}
...
QString TournamentSession::toString(FundingType type)
{
    switch(type)
    {
    case FundingType::Buyin:
        return QObject::tr("Buy-in");
    case FundingType::Rebuy:
        return QObject::tr("Rebuy");
    case FundingType::Addon:
        return QObject::tr("Add-on");
    default:
...
QString TournamentSession::toString(RebalancePolicy policy)
{
    switch(policy)
    {
    case RebalancePolicy::Manual:
        return QObject::tr("Manual");
    case RebalancePolicy::Automatic:
        return QObject::tr("Automatic");
    case RebalancePolicy::Shootout:
        return QObject::tr("Shootout");
    default:
        return QObject::tr("Unknown");
    }
}
...
QString TournamentSession::toString(PayoutPolicy policy)
{
    switch(policy)
    {
    case PayoutPolicy::Automatic:
        return QObject::tr("Automatic");
    case PayoutPolicy::Forced:
        return QObject::tr("Forced");
    case PayoutPolicy::Manual:
        return QObject::tr("Manual");
    default:
        return QObject::tr("Unknown");
    }
...
QString TournamentSession::toString(AnteType type)
{
    ...
    case AnteType::Traditional:
        return QObject::tr("Traditional");
    case AnteType::BigBlind:
        return QObject::tr("Big Blind");
    default:
        return QObject::tr("None");
    }
}
...
{
    if(str == QObject::tr("Buy-in"))
        return FundingType::Buyin;
    if(str == QObject::tr("Rebuy"))
        return FundingType::Rebuy;
    if(str == QObject::tr("Add-on"))
        return FundingType::Addon;
...
{
    if(str == QObject::tr("Manual"))
        return RebalancePolicy::Manual;
    if(str == QObject::tr("Automatic"))
        return RebalancePolicy::Automatic;
    if(str == QObject::tr("Shootout"))
        return RebalancePolicy::Shootout;
...
{
    if(str == QObject::tr("Automatic"))
        return PayoutPolicy::Automatic;
    if(str == QObject::tr("Forced"))
        return PayoutPolicy::Forced;
    if(str == QObject::tr("Manual"))
        return PayoutPolicy::Manual;
...
{
    if(str == QObject::tr("Traditional"))
        return AnteType::Traditional;
    if(str == QObject::tr("Big Blind"))
        return AnteType::BigBlind;
    if(str == QObject::tr("None"))
