...
int test_json()
{
    ...
    int testnumber(0);
    test_assert(test.get_value<int>("testnumber", testnumber), "test object number child not correctly output as int");
    test_assert(testnumber == 1, "test object int child not correctly interpreted");

    double testdouble(0.0);
    test_assert(test.get_value<double>("testnumber", testdouble), "test object number child not correctly output as double");
    test_assert(testdouble == 1.0, "test object double child not correctly interpreted");

    std::string teststring;
    test_assert(test.get_value<std::string>("teststring", teststring), "test object string child not correctly output as string");
